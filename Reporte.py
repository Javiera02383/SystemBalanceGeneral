...
def generar_pdf(registros, total_activos_corrientes, total_activos_no_corrientes, total_pasivos_corrientes, total_pasivos_no_corrientes, total_capital, totalEfectivo,
    ...
    pdf_filename = f"Balance_de_{empresa}_{anio}_Reporte.pdf"
    ...
    def crear_tabla(registros, nombre_subgrupo=None, total_subgrupo=None, nombre_grupo=None, total_grupo=None):
        data = []
    ...
    for grupo, subgrupos in grupos.items():
        ...
        for subgrupo_nombre, registros_subgrupo in subgrupos.items():
            ...
            if registros_subgrupo:
                ...
                content.append(crear_tabla(registros_subgrupo, subgrupo_nombre, total_subgrupo))
                content.append(Spacer(1, 12))
        
        # Mostrar el total del grupo después de todos los subgrupos
        if grupo == "Activos Corrientes":
            content.append(crear_tabla([], None, None, grupo, total_activos_corrientes))
        elif grupo == "Activos No Corrientes":
            content.append(crear_tabla([], None, None, grupo, total_activos_no_corrientes))
        elif grupo == "Pasivos Corrientes":
            content.append(crear_tabla([], None, None, grupo, total_pasivos_corrientes))
        elif grupo == "Pasivos No Corrientes":
            content.append(crear_tabla([], None, None, grupo, total_pasivos_no_corrientes))
        elif grupo == "Capital":
            content.append(crear_tabla([], None, None, grupo, total_capital))
    ...
    content.append(Paragraph(f"_______________              _____________________            ______________________________"))
